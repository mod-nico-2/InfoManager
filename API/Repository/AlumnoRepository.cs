...
namespace DesignAPI.Repository
{
    public class AlumnoRepository : IAlumnoRepository
    {
        ...
        public async Task<AlumnoEntity> GetAsync(int id)
        {
            if (_cache.TryGetValue(AlumnoCacheKey, out ICollection<AlumnoEntity> alumnosCached))
            {
                var alumno = alumnosCached.FirstOrDefault(a => a.DNI.Equals(id));
                if (alumno != null)
                    return alumno;
            }

            return await _context.Alumnos.FirstOrDefaultAsync(a => a.DNI.Equals(id));
        }

        public async Task<bool> ExistsAsync(int id)
        {
            return await _context.Alumnos.AnyAsync(a => a.DNI.Equals(id));
        }
